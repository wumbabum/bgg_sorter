defmodule Web.CollectionLive do
  ...
  def handle_params(%{"username" => username} = params, _url, socket) do
    ...
    require Logger
    Logger.info("handle_params: URL params = #{inspect(params)}")
    Logger.info("handle_params: Parsed filters = #{inspect(filters)}")
    
    current_page = socket.assigns.current_page
    current_filters = socket.assigns.filters
    
    Logger.info("handle_params: Current filters = #{inspect(current_filters)}")
    Logger.info("handle_params: Filters equal? #{filters == current_filters}")
  ...
  def handle_info({:load_collection, username}, socket) do
    ...
    require Logger
    Logger.info("Loading collection for #{username} with client-side filters: #{inspect(socket.assigns.filters)}")
    
    case Core.BggGateway.collection(username, []) do
      {:ok, collection_response} ->
        # Apply client-side filters if any are set
        original_count = length(collection_response.items)
        Logger.info("Collection loaded: #{original_count} items before filtering")
        Logger.info("Applying filters: #{inspect(socket.assigns.filters)}")
        
        filtered_items = apply_filters(collection_response.items, socket.assigns.filters)
        total_items = length(filtered_items)
        
        Logger.info("Filtering complete: #{total_items} items after filtering (#{original_count - total_items} filtered out)")
    ...
    case Core.BggGateway.things(game_ids, []) do
      ...
      {:error, reason} ->
        ...
        socket =
          socket
          |> assign(:collection_loading, false)
          |> assign(:collection_items, basic_items)  # Show basic items
          |> assign(:search_error, error_message)
  ...
  def handle_event("clear_filters", _params, socket) do
    ...
    case socket.assigns.username do
      ...
      username ->
        ...
        url = if socket.assigns.advanced_search do
  ...
  def handle_event("goto_page", %{"page" => page_str}, socket) do
    case Integer.parse(page_str) do
      {page, _} when page > 0 ->
        ...
        url = build_collection_url(username, filters, 
          page: page, 
  ...
  def handle_event("toggle_advanced_search", _params, socket) do
    ...
    case socket.assigns.username do
      ...
      username ->
        ...
        new_url = if new_advanced_search do
          "/collection/#{username}?advanced_search=true"
        else
          "/collection/#{username}"
        end
  ...
  defp merge_collection_with_details(collection_items, detailed_things) do
    ...
    Enum.map(collection_items, fn item ->
      case Map.get(details_map, item.id) do
        nil -> item  # No detailed info found, keep original
  ...
  defp apply_filters(items, filters) do
    require Logger
    Logger.info("Starting filter process with #{length(items)} items and filters: #{inspect(filters)}")
    
    filtered_items = Enum.filter(items, fn item ->
      result = matches_all_filters?(item, filters)
      # Log first few items for debugging
      if length(items) > 0 and item == hd(items) do
        Logger.info("First item '#{item.primary_name}' matches filters: #{result}")
      end
      result
    end)
    
    Logger.info("Filter process complete: #{length(filtered_items)} items matched")
    filtered_items
  end
  ...
  defp matches_filter?(item, :yearpublished_min, min_year) do
    case {parse_integer(item.yearpublished), parse_integer(min_year)} do
      ...
      _ -> true  # Skip filter if data is invalid
    end
  end
  ...
  defp matches_filter?(item, :players, target_players) do
    require Logger
    min_p = parse_integer(item.minplayers)
    max_p = parse_integer(item.maxplayers)
    target = parse_integer(target_players)
    
    result = case {min_p, max_p, target} do
      {min_p, max_p, target} when is_integer(min_p) and is_integer(max_p) and is_integer(target) ->
        matches = target >= min_p and target <= max_p
        Logger.info("Player filter - Game: '#{item.primary_name}' (#{min_p}-#{max_p} players), Target: #{target}, Matches: #{matches}")
        matches
      _ -> 
        Logger.info("Player filter - Game: '#{item.primary_name}' has invalid player data (min: #{inspect(item.minplayers)}, max: #{inspect(item.maxplayers)})")
        true
    end
  ...
  defp matches_filter?(item, :minage, max_minage) do
    case {parse_integer(item.minage), parse_integer(max_minage)} do
      {item_minage, max_minage_int} when is_integer(item_minage) and is_integer(max_minage_int) ->
        item_minage <= max_minage_int  # Game min age should be <= filter (younger or same)
      _ -> true
    end
  end
  ...
  defp matches_filter?(item, :rank, max_rank) do
    case {parse_integer(item.rank), parse_integer(max_rank)} do
      {item_rank, max_rank_int} when is_integer(item_rank) and is_integer(max_rank_int) and item_rank > 0 ->
        item_rank <= max_rank_int  # Lower rank number is better
      _ -> true
    end
  end
  ...
  defp build_collection_url(username, filters, opts \\ []) do
    ...
    query_params = if opts[:advanced_search] do
