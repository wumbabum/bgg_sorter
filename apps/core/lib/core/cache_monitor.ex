defmodule Core.CacheMonitor do
  ...
  @spec freshness_distribution() :: %{
    very_fresh: integer(),  # < 1 day
    fresh: integer(),       # 1-3 days  
    aging: integer(),       # 3-7 days
    stale: integer(),       # > 7 days
    never_cached: integer() # nil last_cached
  }
  ...
  defp get_average_cache_age_days do
    ...
    case result do
      nil -> 0.0
      age_seconds -> age_seconds / (24 * 60 * 60)  # Convert to days
    end
  end
  ...
  defp estimate_storage_size_mb do
    ...
    total_items = get_total_cached_items()
    estimated_size_per_item = 2048  # bytes, rough estimate
